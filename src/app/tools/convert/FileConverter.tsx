...
export default function FileConverter() {
  ...
  const [splitOddEven, setSplitOddEven] = useState(false); // 新增状态
  ...
  const uploadChunk = async (file: File, uploadId: string, index: number, total: number) => {
    ...
    formData.append('file', file.slice(index * CHUNK_SIZE, Math.min((index + 1) * CHUNK_SIZE, file.size)));
    formData.append('uploadId', uploadId);
    formData.append('index', index.toString());
    formData.append('total', total.toString());
    formData.append('fileName', file.name);
    formData.append('makeEven', makeEven.toString());
    formData.append('splitOddEven', splitOddEven.toString()); // 传递参数

    const res = await fetch('/api/proxy/convert/upload-chunk', { method: 'POST', body: formData });
    return res.json();
  };
    ...
    for (const fileItem of waiting) {
      ...
      try {
        for (let i = 0; i < totalChunks; i++) {
          ...
          updateFileStatus(fileItem.uid, { progress: Math.round(((i + 1) / totalChunks) * 100) });
        }
    ...
    const timer = setInterval(async () => {
      try {
        ...
        if (data.status === 'completed') {
          clearInterval(timer);
          updateFileStatus(uid, { status: 'completed', downloadToken: data.downloadToken, outputFileName: data.outputFileName, size: data.outputSize });
          checkProcessingDone();
  ...
  const checkProcessingDone = () => {
    setFileList(prev => {
      if (!prev.some(f => f.status === 'uploading' || f.status === 'processing')) setIsProcessing(false);
      return prev;
    });
  };

  const downloadFile = (file: ExtendedFile) => {
    if (file.jobId && file.downloadToken) {
      window.open(`/api/proxy/convert/download/${file.jobId}?token=${file.downloadToken}`, '_blank');
    }
  };
  ...
  return (
    <Card variant="borderless" style={{ borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.04)' }}>
      <Dragger
        ...
        beforeUpload={(file) => {
          if (!file.name.toLowerCase().endsWith('.docx') && !file.name.toLowerCase().endsWith('.zip')) {
            message.error('仅支持 DOCX 或 ZIP 格式');
            return Upload.LIST_IGNORE;
          }
          setFileList(prev => [...prev, { uid: uuidv4(), name: file.name, size: file.size, status: 'wait', originFileObj: file }]);
          return false;
        }}
      >
      ...
      {fileList.length > 0 && (
        <div style={{ marginTop: 24 }}>
          {fileList.some(f => f.status === 'completed') && (
            <Alert
              message="下载建议"
              description="如果您下载的是大文件或下载速度较慢，建议复制直链并使用多线程下载工具。"
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            ...
            <Space wrap>
              <Space size={4}>
                <Text style={{ fontSize: 13 }}>奇数页补白</Text>
                <Switch size="small" checked={makeEven} onChange={(val) => { setMakeEven(val); if(!val) setSplitOddEven(false); }} disabled={isProcessing} />
              </Space>
              <Tooltip title="开启补白后可选，将结果拆分为奇/偶数页两个文件">
                <Space size={4}>
                  <Text style={{ fontSize: 13, color: !makeEven ? '#bfbfbf' : 'inherit' }}>奇偶页分离</Text>
                  <Switch size="small" checked={splitOddEven} onChange={setSplitOddEven} disabled={isProcessing || !makeEven} />
                </Space>
              </Tooltip>
              ...
              {fileList.length > 1 && fileList.some(f => f.status === 'completed') && (
                <Button icon={<DownloadOutlined />} onClick={() => fileList.filter(f => f.status === 'completed').forEach((item, index) => setTimeout(() => downloadFile(item), index * 800))}>一键下载</Button>
              )}
              ...
              <List.Item
                ...
                actions={[
                  file.status === 'completed' && (
                    <Space key="ops">
                      <Tooltip title="复制直链"><Button type="text" size="small" icon={<CopyOutlined />} onClick={() => {
                        const link = `${window.location.origin}/api/proxy/convert/download/${file.jobId}?token=${file.downloadToken}`;
